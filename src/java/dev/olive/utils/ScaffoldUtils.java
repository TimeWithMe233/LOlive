...
import static dev.olive.utils.player.RotationUtil.getVectorForRotation;
...
public class ScaffoldUtils {
    ...
    public static float[] faceBlock(final BlockPos blockPos) {
            ...
            for(double ySearch = 0.1D; ySearch < 0.9D; ySearch += 0.1D) {
                for (double zSearch = 0.1D; zSearch < 0.9D; zSearch += 0.1D) {
                    ...
                    if (obj.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && obj.getBlockPos().equals(blockPos) &&
                        ...
                        if (vecRotation == null || Client.instance.getRotationManager().getRotationDifference(vecRotation, new Vector2f(PlayerUtil.getMoveYaw(mc.thePlayer.rotationYaw) - 180f, Client.instance.getRotationManager().lastServerRotation.y)) > Client.instance.getRotationManager().getRotationDifference(currentVec, new Vector2f(PlayerUtil.getMoveYaw(mc.thePlayer.rotationYaw) - 180f, Client.instance.getRotationManager().lastServerRotation.y))) {
        ...
        final Rotation rotation = new Rotation(PlayerUtil.getMoveYaw(mc.thePlayer.rotationYaw) - 180f, MathUtils.getRandomInRange(79.5f, 83.5f));
        final Vec3 rotationVector = getVectorForRotation(rotation);
            ...
            for(double ySearch = 0.1D; ySearch < 0.9D; ySearch += 0.1D) {
                for (double zSearch = 0.1D; zSearch < 0.9D; zSearch += 0.1D) {
                    ...
                    if (obj.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && obj.getBlockPos().equals(blockPos) && obj.sideHit == facing) {
                        ...
                        if (vecRotation == null || Client.instance.getRotationManager().getRotationDifference(currentVec, preferRot) < Client.instance.getRotationManager().getRotationDifference(vecRotation, preferRot))
                            vecRotation = currentVec;
                    }
                }
            }
