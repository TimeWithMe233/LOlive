...
public class AttackUtil implements IMinecraft {
    ...
    public static final TimerUtil combatTimer = new TimerUtil();
    ...
    public static EntityLivingBase getTarget(double range, String sort) {
        final KillAura targets = Client.instance.getModuleManager().getModule(KillAura.class);
        final Teams teams = Client.instance.getModuleManager().getModule(Teams.class);
        final FriendManager friendManager = Client.instance.friendManager;
        return getTarget(range, sort, teams.getState(), targets.mobValue.getValue(), targets.animalValue.getValue(), targets.playerValue.getValue(), friendManager.isFriend(String.valueOf(targets.target)));
    }
    ...
    public static EntityLivingBase getTarget(double range, String sort, boolean team, boolean mobs, boolean animals, boolean players, boolean friends) {
        ...
        for (Entity entity : mc.theWorld.getLoadedEntityList().stream().filter(Objects::nonNull).collect(Collectors.toList())) {
            if (entity instanceof EntityLivingBase) {
                ...
                if (entity instanceof EntityPlayer && !team && Teams.isSameTeam((EntityPlayer) entity)) continue;
                if (entity instanceof EntityPlayer && !friends && Client.instance.getFriendManager().isFriend(String.valueOf((EntityPlayer) entity))) continue;

                if (mc.thePlayer.getDistanceToEntity(entity) > range) continue;
                if (Client.instance.getModuleManager().getModule(AntiBot.class).getState() && Client.instance.getModuleManager().getModule(AntiBot.class).isServerBot((EntityLivingBase) entity)) continue;
