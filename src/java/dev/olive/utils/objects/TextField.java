...
public class TextField extends Gui {
    ...
    public TextField(RapeMasterFontManager font, float x, float y, float par5Width, float par6Height) {
        this.font = font;
        this.xPosition = x;
        this.yPosition = y;
        ...
        if (this.getVisible()) {
            ...
            if (this.isDrawingBackground()) {
                ...
                Gui.drawRect2(xPosition, yPosition + height - rectHeight, width, rectHeight,
                        ColorUtil.interpolateColor(focusedTextColor, unfocusedTextColor, (float) textColor.getOutput()));
            }

            textColor.setDirection(isFocused() ? Direction.BACKWARDS : Direction.FORWARDS);
            if (backgroundText != null) {
                Color backgroundTextColor = ColorUtil.applyOpacity(ColorUtil.applyOpacity(unfocusedTextColor, textAlpha), (float) textColor.getOutput());
                if (backgroundText.equals("Search")) {
                    FontManager.iconFont16.drawString("B", xPos + 1.5f, this.yPosition + FontManager.iconFont16.getMiddleOfBox(getHeight()), ColorUtil.applyOpacity(unfocusedTextColor, textAlpha).getRGB());
                    xPos += 15;
                }

                if (text.equals("") && !textColor.finished(Direction.BACKWARDS)) {
                    font.drawString(backgroundText, xPos, yPos, backgroundTextColor.getRGB());
                }
            }
            ...
            if (canShowCursor) {
                ...
                Gui.drawRect2(k1 + 1, yPos - 2, .5f, font.getHeight() + 3,
                        ColorUtil.applyOpacity(textColorWithAlpha, (float) cursorBlinkAnimation.getOutput()).getRGB());
            }
