...
public class ParticleEngine {
    ...
    public void render(float mouseX, float mouseY) {
        ...
        for (Particle p : particles) {
            ...
            GLUtils.startSmooth();
            RenderUtil.drawBorderedCircle(p.x + MathHelper.sin(p.ticks / 2) * 50 + -xOffset / 5, sr.getScaledHeight() - ((p.ticks * p.speed) * p.ticks / 10 + yOffset / 5), p.radius * (p.opacity / 32), c.getRGB(), c.getRGB());
            GLUtils.endSmooth();
