...
public class GuiWelcome extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        fontwel.drawString("Welcome back to " + Client.name, sr.getScaledWidth() / 2 -  fontwel.getStringWidth("Welcome back to " + Client.name) / 2,sr.getScaledHeight() / 2 - 15 , new Color(255, 255, 255).getRGB());
        fontwel2.drawString(Client.instance.user + "! 屏幕各处随意点击进入菜单", sr.getScaledWidth() / 2 -  fontwel2.getStringWidth(Client.instance.user + "! 屏幕各处随意点击进入菜单") / 2,sr.getScaledHeight() / 2 + 10, new Color(255, 255, 255).getRGB());
        ...
        if (timer.hasReached(4000) || skiped) {
            rev = true;
            if (anim2 >= width - 5) {
                mc.displayGuiScreen(new CustomMainMenu(true));
            }
        } else {
            rev = false;
        }
