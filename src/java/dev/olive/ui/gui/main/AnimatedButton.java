...
public class AnimatedButton {
    ...
    public AnimatedButton(FontRenderer font, String text,String icons,double posX, double posY,Runnable action) {
        this.posX = posX;
        this.posY = posY;
    ...
    public void updateAndDraw(int mouseX, int mouseY) {
        if (isIntroPlayed) {
            ...
            if (hoverAnim2 > 5) {
                FontManager.NovICON64.drawString(icons, (float) (posX) - 10f - (icons.equals("A") ? 10 : 0), (float) posY - 20, new Color(190, 190, 190, Math.max((int) hoverAnim2,0)).getRGB());
            }


            font.drawString(text, (int) (posX + hoverAnim), (int) posY, new Color(255, 255, 255, (int) animColor).getRGB());
