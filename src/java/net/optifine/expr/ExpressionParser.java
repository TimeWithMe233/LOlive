...
public class ExpressionParser {
    ...
    private FunctionType getFunctionType(Token token, Deque<Token> deque) throws ParseException {
        ...
        if (tokenPeek != null && tokenPeek.getType() == TokenType.BRACKET_OPEN) {
            FunctionType functiontype1 = FunctionType.parse(tokenPeek.getText());
            checkNull(functiontype1, "Unknown function: " + tokenPeek);
    ...
    private IExpression[] parseExpressions(Deque<Token> deque) throws ParseException {
        ...
        while (true) {
            Deque<Token> dequeGroup = getGroup(deque, TokenType.COMMA, false);
            IExpression iexpression = this.parseInfix(dequeGroup);
