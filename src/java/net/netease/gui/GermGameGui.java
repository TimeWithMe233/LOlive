...
import dev.olive.utils.render.animation.impl.RippleAnimation;
...
public class GermGameGui extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        FontManager.font40.drawStringWithShadow("Olive Germ Game", this.x + 20.0f, this.y + 8.0f, -1);
        int count = 0;
        float offsetX = 0.0f;
        float offsetY = 0.0f;
        for (GermGameElement element : this.elements) {
            ...
            if (element.getDefaultImage() != null && element.getGifImage() != null) {
                ...
                RoundedUtil.drawRoundTextured(x, y, width, height, 5.0f, 1.0f);
            }
            if (hovering && !this.switchScreenAnim.isState()) {
                float textOffsetY = 0.0f;
                for (String hoverDos : element.getHoverDos()) {
                    font18.drawStringWithShadow(hoverDos, mouseX + 15, (float) (mouseY - (font18.getHeight() + 4) * 2) + textOffsetY, -1);
                    textOffsetY += (float) (font18.getHeight() + 4);
                }
            }
            if (++count % 3 == 0) {
        ...
        if (this.switchScreenAnim.isState() || !this.switchScreenAnim.isDone()) {
            RenderUtil.startGlScissor((int) originalX, (int) this.y, (int) this.width, (int) this.height);
            RoundedUtil.drawRound(doubleX + 10.0f, this.y + 10.0f, 20.0f, 20.0f, 10.0f, new Color(0, 0, 0, 80));
            this.backButtonRipple.draw(() -> RoundedUtil.drawRound(doubleX + 10.0f, this.y + 10.0f, 20.0f, 20.0f, 10.0f, Color.WHITE));
            this.backHoverAnim.setState(RenderUtil.isHovering(doubleX + 20.0f - (float) font18.getStringWidth("Back") / 2.0f, this.y + 20.0f - (float) (font18.getHeight() / 2), font18.getStringWidth("Back"), font18.getHeight(), mouseX, mouseY));
            ...
            font18.drawStringWithShadow("Back", doubleX + 20.0f - (float) font18.getStringWidth("Back") / 2.0f, this.y + 22.0f - (float) (font18.getHeight() / 2), hoverColor.getRGB());
            font18.drawCenteredStringWithShadow("Sub Game", doubleX + this.width / 2.0f, this.y + 10.0f, -1);
            if (this.currentElement != null) {
                ...
                for (GermGameSubElement element : this.currentElement.getSubElements()) {
                    ...
                    RoundedUtil.drawRound(x, y, width, height, 7.0f, hovColor);
                    font18.drawCenteredStringWithShadow(element.getName(), x + width / 2.0f, y + 12.0f, -1);
                    float descOffsetY = 0.0f;
                    for (String s : element.getDesc()) {
                        FontManager.font10.drawStringWithShadow(s, x + 5.0f, y + height - (float) (element.getDesc().size() * 10) + descOffsetY, -1);
                        descOffsetY += 10.0f;
                    }
                    element.getAnimation().draw(() -> RoundedUtil.drawRound(x, y, width, height, 7.0f, Color.WHITE));
                    startX += width + 20.0f;
                }
                ...
                RoundedUtil.drawRound(doubleX + scrollX, this.y + this.height - 8.0f, scrollWidth, 5.0f, 2.0f, new Color(0, 0, 0, 150));
            }
            RenderUtil.stopGlScissor();
        }
        this.x = originalX;
