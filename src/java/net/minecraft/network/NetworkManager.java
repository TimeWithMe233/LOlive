...
import dev.olive.utils.GetC03StatusUtil;
import dev.olive.utils.PacketStoringComponent;
...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    public void sendPacketFinal(Packet packetIn) {
        ...
        this.dispatchPacket((Packet)event.getPacket(), null);
    }
        ...
        if (this.channel.isOpen()) {
            try {
                ...
                if (p_channelRead0_2_ instanceof S3FPacketCustomPayload) {
                    final PacketReceiveEvent packetReceive = new PacketReceiveEvent(p_channelRead0_2_, this.packetListener);
                    Client.instance.eventManager.call(packetReceive);
    ...
    {
        if (!noEvent) {
            EventPacket event = new EventPacket(EventPacket.EventState.SEND, packetIn, this.packetListener);
    ...
    public void sendPacket(Packet packetIn, GenericFutureListener<? extends Future<? super Void>>
            listener, GenericFutureListener<? extends Future<? super Void>>... listeners) {

        if (Client.instance != null && Client.instance.packetManager != null && Client.instance.packetManager.handle(packetIn)) {
            return;
        }
        if (this.isChannelOpen()) {
            this.flushOutboundQueue();
    ...
    private void dispatchPacket(final Packet inPacket, final GenericFutureListener<? extends Future<? super Void>>[] futureListeners) {
        EventPacket sendEvent = new EventPacket(EventPacket.EventState.SEND, inPacket, this.packetListener);

        if (mc.thePlayer != null) {
            Client.instance.eventManager.call(sendEvent);
        }
...
/*
    ...
    public void sendUnregisteredPacketWithoutEvent(final Packet packetIn) {
