...
import dev.olive.module.impl.player.XRay;
...
public class BlockModelRenderer {
    ...
    public boolean renderModel(IBlockAccess blockAccessIn, IBakedModel modelIn, IBlockState blockStateIn, BlockPos blockPosIn, WorldRenderer worldRendererIn, boolean checkSides) {
        ...
        try {
            ...
            if (Client.instance.moduleManager.getModule(XRay.class).getState()) {
                for (final EnumFacing enumfacing : EnumFacing.VALUES) {
                    final List<?> list = (List<?>) modelIn.getFaceQuads(enumfacing);
                    if (!list.isEmpty()) {
                        final BlockPos blockpos = blockPosIn.offset(enumfacing);
                        if ((!checkSides || block.shouldSideBeRendered(blockAccessIn, blockpos, enumfacing)) && XRay.isEnabled && XRay.showESP()) {
                ...
                return this.renderModelAmbientOcclusion(blockAccessIn, modelIn, block, blockStateIn, blockPosIn, worldRendererIn, checkSides);
            }
    ...
    public boolean renderModelAmbientOcclusion(IBlockAccess p_renderModelAmbientOcclusion_1_,
                                               IBakedModel p_renderModelAmbientOcclusion_2_,
                                               Block p_renderModelAmbientOcclusion_3_,
                                               IBlockState p_renderModelAmbientOcclusion_4_,
                                               BlockPos p_renderModelAmbientOcclusion_5_,
                                               WorldRenderer p_renderModelAmbientOcclusion_6_,
            ...
            if (!list.isEmpty()) {
                ...
                if (!p_renderModelAmbientOcclusion_7_ || p_renderModelAmbientOcclusion_3_.shouldSideBeRendered(p_renderModelAmbientOcclusion_1_, blockpos, enumfacing)) {
                    if (XRay.isEnabled && XRay.showESP()) {
                        Block block = Minecraft.getMinecraft().theWorld.getBlockState(p_renderModelAmbientOcclusion_5_).getBlock();

                        if (block instanceof BlockOre || block instanceof BlockRedstoneOre) {
                            double x = p_renderModelAmbientOcclusion_5_.getX(), y = p_renderModelAmbientOcclusion_5_.getY(), z = p_renderModelAmbientOcclusion_5_.getZ();

                            if (Minecraft.getMinecraft().thePlayer.getDistance(x, z) <= XRay.getDistance()) {
                    ...
                    if (renderenv == null) {
                        renderenv = new RenderEnv(p_renderModelAmbientOcclusion_4_, p_renderModelAmbientOcclusion_5_);
                    }
                    ...
                    this.renderQuadsSmooth(p_renderModelAmbientOcclusion_1_, p_renderModelAmbientOcclusion_4_, p_renderModelAmbientOcclusion_5_, p_renderModelAmbientOcclusion_6_, list, renderenv);
                    flag = true;
                }
            }
        }
        ...
        if (!list1.isEmpty()) {
            ...
            this.renderQuadsSmooth(p_renderModelAmbientOcclusion_1_, p_renderModelAmbientOcclusion_4_, p_renderModelAmbientOcclusion_5_, p_renderModelAmbientOcclusion_6_, list1, renderenv);
            flag = true;
        }

        if (renderenv != null && Config.isBetterSnow() && !renderenv.isBreakingAnimation() && BetterSnow.shouldRender(p_renderModelAmbientOcclusion_1_, p_renderModelAmbientOcclusion_4_, p_renderModelAmbientOcclusion_5_)) {
