...
public class PlayerControllerMP {
    ...
    // 修正命名并初始化
    private long lastBlockClickTime = 0L;
    ...
    public boolean clickBlock(BlockPos loc, EnumFacing face) {
        if (this.currentGameType.isAdventure()) {
            ...
            if (clickTimestamps.size() >= 2) {
                long windowSpan = currentTime - clickTimestamps.getFirst();
                blockRate = (clickTimestamps.size() - 1) * 1000.0 / windowSpan;
            }
            ...
            mc.theWorld.sendBlockBreakProgress(mc.thePlayer.getEntityId(), loc, 0);
            if (!this.mc.thePlayer.isAllowEdit()) {
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
        Client.instance.eventManager.call(new EventAttack(targetEntity, true));
        ...
        Client.instance.eventManager.call(new EventAttack(targetEntity, false));
    ...
    public ItemStack windowClick(int windowId, int slotId, int mouseButtonClicked, int mode, EntityPlayer playerIn) {
        ...
        this.netClientHandler.addToSendQueueUnregistered(new C0EPacketClickWindow(windowId, slotId, mouseButtonClicked, mode, itemstack, short1));
        return itemstack;
    }
