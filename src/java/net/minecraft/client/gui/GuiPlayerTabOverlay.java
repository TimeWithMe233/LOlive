...
public class GuiPlayerTabOverlay extends Gui {
    ...
    public String getPlayerName(NetworkPlayerInfo networkPlayerInfoIn) {
        String str = networkPlayerInfoIn.getDisplayName() != null ? networkPlayerInfoIn.getDisplayName().getFormattedText() : ScorePlayerTeam.formatPlayerName(networkPlayerInfoIn.getPlayerTeam(), networkPlayerInfoIn.getGameProfile().getName());
       /* if (IRC.client.isConnected()) {
            for (IRCUser user : IRC.client.getOnlineUsers()) {
                if (networkPlayerInfoIn.getGameProfile().getName().equals(user.getIngamename())) {
                    str = String.format(EnumChatFormatting.GREEN + "[%s] %s", user.getUsername(), EnumChatFormatting.RESET + str);
                    String rank = user.getRank();
                    if (rank != null && rank.length() > 0) {
                        str = String.format(EnumChatFormatting.GREEN + "[%s] %s", user.getRank(), str);
                    }
                    break;
                }
            }
...
*/
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn) {
        ...
        if (this.header != null) {
            list1 = this.mc.fontRendererObj.listFormattedStringToWidth(this.header.getFormattedText().replace("花雨庭", "花雨庭").replace("❀", "❀").replace("www.mchyt.com", "www.mchyt.com"), width - 50);
        ...
        if (this.footer != null) {
            list2 = this.mc.fontRendererObj.listFormattedStringToWidth(this.footer.getFormattedText().replace("www.mchyt.com", "www.mchyt.com"), width - 50);
