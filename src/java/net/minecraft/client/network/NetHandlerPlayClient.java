...
import dev.olive.event.impl.events.EventPacketCustom;
import dev.olive.event.impl.events.VelocityEvent;
...
import dev.olive.module.impl.misc.Protocol;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn) {
        ...
        Protocol protocol = Client.instance.getModuleManager().getModule(Protocol.class);

        if (protocol.mode.is("HYT")) {
            PacketUtil.sharkBee();
        }
    }
    public void sendPacketNoEvent(Packet packetIn) {
        this.netManager.sendPacket(packetIn, true);
    }

    public void addToSendQueueUnregisteredNoEvent(final Packet p_147297_1_) {
    ...
    public void handleEntityVelocity(S12PacketEntityVelocity packetIn) {
        ...
        if (entity != null) {
            VelocityEvent event = new VelocityEvent((double)packetIn.getMotionX() / 8000.0, (double)packetIn.getMotionY() / 8000.0, (double)packetIn.getMotionZ() / 8000.0);
        ...
        if (this.gameController.theWorld.isBlockLoaded(packetIn.getPos())) {
            ...
            if (tileentity instanceof TileEntitySign) {
                ...
                if (tileentitysign.getIsEditable()) {
                    IChatComponent[] lines = packetIn.getLines();
                    for (int i = 0; i < 4; i++) {
                        tileentitysign.signText[i] = this.gameController.fontRendererObj.getStringWidth(lines[i].getUnformattedText()) > 90 ? new ChatComponentText("[Malformed text]") : lines[i];
                    }
    ...
    public void handleCustomPayload(S3FPacketCustomPayload packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        PacketProcessor.INSTANCE.handleS3F(packetIn);
        Client.instance.eventManager.call(new EventPacketCustom(packetIn));
