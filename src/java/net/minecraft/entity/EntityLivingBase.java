...
import dev.olive.event.impl.events.EventJump;
import dev.olive.event.impl.events.MoveMathEvent;
import dev.olive.module.impl.move.NoLiquid;
...
public abstract class EntityLivingBase extends Entity {
    ...
    //  }
    private int getArmSwingAnimationEnd() {
        int speed = Client.instance.moduleManager.getModule(BlockAnimation.class).getState() ? (20 - BlockAnimation.SpeedSwing.getValue().intValue()) : 6;
    ...
    protected void jump() {
        ...
        if (this instanceof EntityPlayerSP) {
            Client.instance.eventManager.call(jumpEvent);
    ...
    public void onLivingUpdate() {
        ...
        if (this.isJumping) {
            if (this.isInWater() && !NoLiquid.shouldCancelWater) {
                this.updateAITick();
            } else if (this.isInLava() && !NoLiquid.shouldCancelWater) {
                this.handleJumpLava();
