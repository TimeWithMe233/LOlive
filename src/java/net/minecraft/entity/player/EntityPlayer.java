...
import dev.olive.utils.SmoothCameraComponent;
import dev.olive.utils.render.waveycapes.config.Config;
import dev.olive.utils.render.waveycapes.sim.StickSimulation;
...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    /* Player blockrate values */
    private CopyOnWriteArrayList<Boolean> isBlockings = new CopyOnWriteArrayList<>();
    ...
    public boolean enableCameraYOffset;
    public double cameraYPosition;
    ...
    public boolean isBlocking() {
        return KillAura.isBlocking;
    }

    private void addBlock(boolean blocking) {
        if (isBlockings.size() == 20) {
            isBlockings.remove(0);
        }
        isBlockings.add(blocking);
    }

    /***
     * @param roundup 保留 {roundup} 位小数
     * @return 格挡率 如: 0.80
     */
    public double getBlockRate(int roundup) {
        return Math.round(getBlockRate() * (10.0 * roundup)) / (10.0 * roundup);
    }

    public double getBlockRate() {
        if (isBlockings.isEmpty()) return 0.0;
        else return (double) isBlockings.stream().filter((item) -> item).count() / isBlockings.size();
    }
    ...
    public void onUpdate() {
        ...
        try {
            if (!(this.inventory.getCurrentItem().getItem() instanceof ItemSword)) {
                addBlock(false);
            } else {
                addBlock(this.isBlocking());
            }
    ...
    public float getEyeHeight() {
        ...
        if (!SmoothCameraComponent.stopWatch.check(60)) {
