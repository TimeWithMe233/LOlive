...
import dev.olive.event.impl.events.PostStrafeEvent;
...
import dev.olive.utils.vector.Vector3d;
...
public abstract class Entity implements ICommandSender {
    ...
    public float getClosestDistanceToBBox(AxisAlignedBB boundingBox) {
        Vec3 eyes = this.getPositionEyes(1F);
        Vec3 pos = RotationUtil.getClosestPoint(eyes, boundingBox);
    ...
    public MovingObjectPosition rayTraceCustom(double blockReachDistance, float yaw, float pitch, float partialTicks) {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        EventStrafe event = new EventStrafe(strafe, forward, friction, movingYaw);
        ...
        if (this == Minecraft.getMinecraft().thePlayer) {
            final PostStrafeEvent event2 = new PostStrafeEvent();

            Client.instance.eventManager.call(event2);
        }
    }
    ...
    public float getClosestDistanceToEntity(Entity entityIn) {
        Vec3 eyes = this.getPositionEyes(1F);
        Vec3 pos = RotationUtil.getClosestPoint(eyes, entityIn.getEntityBoundingBox());
        double xDist = Math.abs(pos.xCoord - eyes.xCoord);
        double yDist = Math.abs(pos.yCoord - eyes.yCoord);
        double zDist = Math.abs(pos.zCoord - eyes.zCoord);
        return (float) Math.sqrt(xDist * xDist + yDist * yDist + zDist * zDist);
    ...
    public float getDistanceToEntity(Entity entityIn) {
       ...
       public double getDistance(Entity entity) {
             double x = this.posX - entity.getPosition().getX();
             double y = this.posY - entity.getPosition().getY();
             double z = this.posZ - entity.getPosition().getZ();
    ...
    public Vec3 getCustomLook(final float partialTicks, final float yaw, final float pitch) {
     ...
     public double getDistanceSq(Entity entity) {
          return entity.getDistanceSq(this.posX, this.posY, this.posZ);
          }
